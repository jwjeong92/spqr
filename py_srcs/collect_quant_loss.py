...
@torch.no_grad()
def quant_sensitivity_with_calib(model, dataloader, args, device):
    ...
    for i in range(len(layers)):
        ...
        for names in seqeuntial:
            ...
            for sublayer_name in subset:
                ...
                quant_loss[i][sublayer_name] = spqr_handler[sublayer_name].collect_quant_loss(
...
def get_quant_loss(model, args, device):
    tick = time.time()
    print("Loading data ...")
    ...
    results = quant_sensitivity_with_calib(model, dataloader, args, device)
...
def main():
    ...
    results = get_quant_loss(model, args, device)

    df = pd.DataFrame(results)
    if args.groupsize is None:
        results_name = f'opt-125m-w{args.wbits}.pt'
    else:
        results_name = f'opt-125m-w{args.wbits}-gs{args.groupsize}.pt'
    folder_name = '/raid/jwjeong/quant_results'
